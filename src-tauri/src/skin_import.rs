...
fn validate_skin_dir(dir: &Path) -> Result<serde_json::Value, String> {
    ...
    for (state, anim) in animations {
        ...
        if !file_path.exists() {
            return Err(format!("动画 {} 引用的文件不存在: {}", state, file));
        }
    }
...
mod tests {
    ...
    fn validate_skin_dir_empty_animations() {
        ...
        fs::write(
            ...
            r#"{"name":"X","format":"sprite","size":{"width":48,"height":48},"animations":{}}"#,
        )
        .unwrap();
        let err = validate_skin_dir(tmp.path()).unwrap_err();
    ...
    fn validate_skin_dir_missing_animation_file() {
        ...
        fs::write(
            tmp.path().join("skin.json"),
            r#"{"name":"X","format":"sprite","size":{"width":48,"height":48},"animations":{"idle":{"file":"missing.png"}}}"#,
        )
        .unwrap();
        let err = validate_skin_dir(tmp.path()).unwrap_err();
    ...
    fn validate_skin_dir_missing_file_field() {
        ...
        fs::write(
            tmp.path().join("skin.json"),
            r#"{"name":"X","format":"sprite","size":{"width":48,"height":48},"animations":{"idle":{}}}"#,
        )
        .unwrap();
        let err = validate_skin_dir(tmp.path()).unwrap_err();
