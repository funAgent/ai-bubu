...
import type { PeerInfo, SkinManifest, MovementState, SkinAnimationConfig } from '../types'
...
function resolveAnimation(
  ...
  if (m.animations[state]) return { state, config: m.animations[state]! }
  const visited = new Set<MovementState>([state])
  let fb = m.fallback?.[state]
  while (fb && !visited.has(fb)) {
    visited.add(fb)
    if (m.animations[fb]) return { state: fb, config: m.animations[fb]! }
    fb = m.fallback?.[fb]
  }
  if (m.animations.idle) return { state: 'idle', config: m.animations.idle! }
  return null
}
...
const capsuleSize = computed(() => {
  ...
  return { width: `${s}px`, height: `${s}px`, fontSize: `${Math.max(9, Math.round(11 * props.scale))}px` }
})
...
const tooltipText = computed(() => {
  ...
  const state = t(stateKeys[props.peer.movementState] || 'stateIdle')
...
watch(() => props.peer.petSkin, (id) => loadManifest(id), { immediate: false })
